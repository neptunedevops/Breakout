...
for row in range(rows):
    for column in range(columns):
        ...
        #I want this idea: “bricks is a list of brick-objects, and each brick-object is a dictionary”
        brick = brick_memory = {"what_colour_am_I?" : "blue",
                                "How_many_times_have_I_been_hit?" : "0",
                                "Am_I_still_alive" : "yes " }
        print(brick)
...
#balls colliding with bricks
    balls_hit_bricks = True
    for brick in bricks:
        if white_circle_1.colliderect(brick):
            y *= -1
            break 
        if white_circle_2.colliderect(brick):
            y2 *= -1
            break
        if red_circle_1.colliderect(brick):
            red_y *= -1
            break
        if red_circle_2.colliderect(brick):
            red_y2 *= -1
            break
